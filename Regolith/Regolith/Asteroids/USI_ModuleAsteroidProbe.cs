...
{
    public class USI_ModuleAsteroidProbe : PartModule
    {
        [KSPField]
        ...
        private void RunAnalysis()
        {
            ...
            if (_potato != null)
            {
                ...
                if (makeupInfo.Any())
                {
                    ...
                    var resList = _potato.Modules.OfType<USI_ModuleAsteroidResource>().Where(p => p.analysisComplete == false).ToList();
                    var science = 0f;
                    ...
                    foreach (var res in resList)
                    {
                        ...
                        var pi =
                            part.Modules.OfType<USI_ModuleProbeData>().FirstOrDefault(p => p.resourceName == res.resourceName);
                        var thisres =
                            part.Modules.OfType<USI_ModuleAsteroidResource>().FirstOrDefault(x => x.resourceName == res.resourceName);
